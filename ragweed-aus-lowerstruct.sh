#####
# RAGWEED 2024
# LOWERSTRUCT (HACKED LOSTRUCT FOR ANGSD)
#####

# ragweed aus lowerstruct
cd ~/scratch/ragweed/
...
setwd("~/scratch/ragweed/lostruct2/")

CHRS = read.table("~/scratch/ragweed/ref/chromo-length.txt", header = F)

#hbs = as.data.frame(read.table("~/scratch/ragweed/haploblocks-6-1-23.txt", header = F))
...
ggsave(plot_grid(mds.i.c.i.j.plot, mds.j.c.i.j.plot, ncol = 1, align = "hv"),
	file = paste0("~/scratch/ragweed/figs/", CHRS$V1[n], "-", i, "-", j, "-mdsmans.png"),
	device = "png",
	width = 1920,
	height = 240 * 2,
	units = "px",
